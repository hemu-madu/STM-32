...
int main(void)
{
  ...
  uartMutex = osMutexCreate(osMutex(uartMutex));   // ✅ Create mutex
  ...
  osThreadDef(SenderTask, StartSenderTask, osPriorityNormal, 0, 256);
  SenderTaskHandle = osThreadCreate(osThread(SenderTask), NULL);

  /* definition and creation of ReceiverTask */
  osThreadDef(ReceiverTask, StartReceiverTask, osPriorityAboveNormal, 0, 256);
...
void StartSenderTask(void const * argument)
{
  ...
  char msg[] = "[Sender] Semaphore Given!\r\n";

  for(;;)
  {
    osDelay(2000);

    xSemaphoreTake(uartMutex, portMAX_DELAY);   // 🔒 Lock UART
    HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
    xSemaphoreGive(uartMutex);                  // 🔓 Unlock UART

    xSemaphoreGive(myBinarySem);                // Signal Receiver
  }
...
void StartReceiverTask(void const * argument)
{
  ...
  char msg[] = "[Receiver] Got Semaphore! LED Toggled!\r\n";

  for(;;)
  {
    if(xSemaphoreTake(myBinarySem, portMAX_DELAY) == pdTRUE)
    {
      xSemaphoreTake(uartMutex, portMAX_DELAY); // 🔒 Lock UART
      HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
      xSemaphoreGive(uartMutex);                // 🔓 Unlock UART
