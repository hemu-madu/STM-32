...
int main(void)
{
  ...
  osThreadDef(TxTask, StartTxTask, osPriorityNormal, 0, 256);
  TxTaskHandle = osThreadCreate(osThread(TxTask), NULL);

  /* definition and creation of RxTask */
  osThreadDef(RxTask, StartRxTask, osPriorityNormal, 0, 256);
...
void StartTxTask(void const * argument)
{
  ...
  char txMsg[] = "Hello from STM32 FreeRTOS!\r\n";
...
void StartRxTask(void const * argument)
{
  ...
  for(;;)
  {
    // Block until 1 byte arrives
    if(HAL_UART_Receive(&huart2, &rxByte, 1, HAL_MAX_DELAY) == HAL_OK)
    {
      snprintf(echoMsg, sizeof(echoMsg), "Got: %c\r\n", rxByte);
      HAL_UART_Transmit(&huart2, (uint8_t*)echoMsg, strlen(echoMsg), HAL_MAX_DELAY);
    }
  }
